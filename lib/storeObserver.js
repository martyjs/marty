...
class StoreObserver {
  ...
  listenToStore(store, component) {
    ...
    log.trace(
      ...
      'component  (' + component.__id + ') is listening to the', storeDisplayName, 'store'
    );
  ...
  onStoreChanged(state, store, component) {
    log.trace(
      store.displayName + ' store has changed. The ' +
      component.displayName + ' component ( ' + component.__id + ') is updating'
    );

    if (component._lifeCycleState === 'UNMOUNTED') {
      log.warn(
        'Warning: Trying to set the state of ' + component.displayName +
        ' component (' + component.__id + ') which is unmounted'
      );
    } else {
