...
function HttpStateSource(mixinOptions) {
  ...
  var mixin = {
    ...
    request: function (req) {
      ...
      var self = this;
      beforeRequest(self, req);

      return fetch(req.url, req).then(function (res) {
        return afterRequest(self, res);
      });
    }
  };

  function beforeRequest(mixin, req) {
    _.each(getMiddleware('before'), function (middleware) {
      try {
        middleware.before.call(mixin, req);
      } catch (e) {
